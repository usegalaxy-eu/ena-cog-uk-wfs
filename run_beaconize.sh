...
if [ -s "$WORKDIR/$JOB_YML" ]; then
    ...
    planemo -v run d4b0444778f0519c "$WORKDIR/$JOB_YML" --history_name "$SOURCE_HISTORY_NAME"" Viral Beacon export" --galaxy_url 'https://usegalaxy.eu' --galaxy_user_key $API_KEY --engine external_galaxy 2>&1 > /dev/null | grep -o 'GET /api/histories/[^?]*\?' > "$WORKDIR/run_info.txt"
fi
