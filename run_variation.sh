...
python bioblend-scripts/check_history.py -g "$GALAXY_SERVER" -a $API_KEY -p 0.67 --dataset-marker 'Final (SnpEff-) annotated variants' $PREVIOUS_HISTORY &&
...
if [ -s "$WORKDIR/$JOB_YML" ]; then
    ...
    (while [ ! -s "$WORKDIR/run_info.txt" ]; do sleep 60; done; DEST_HISTORY_ID=$(grep -m1 -o 'histories/[^?]*' "$WORKDIR/run_info.txt" | cut -d / -f 2) &&  python bioblend-scripts/tag_history.py $SOURCE_HISTORY_ID --dataset-id $ENA_LINKS -g "$GALAXY_SERVER" -a $API_KEY -t $BOT_RESPONSE -r $BOT_RESERVE) &
    ...
    python bioblend-scripts/tag_history.py $SOURCE_HISTORY_ID --dataset-id $ENA_LINKS -g "$GALAXY_SERVER" -a $API_KEY -t $BOT_RESERVE &&
    # run the WF
